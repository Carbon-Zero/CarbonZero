CarbonZero Core version 3.0.2 is now available from:
...
CarbonZero Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zcarbon spends, automint calculation adjustments, and other various updates/fixes.

zcarbon spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zcarbon mint operation (zcarbon spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzcarbon` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzcarbon=0` at the command-line when starting the client, or add `backupzcarbon=0` to their `carbonzero.conf` file.

zcarbon Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zcarbon mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zcarbon Spending Fix
---------------------
The size of zcarbon spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zcarbon Transaction Recovery
---------------------
Due to the aforementioned issue with zcarbon spending, users may find that their attempted spends are now conflicted and zcarbon balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the CarbonZero address as it's first parameter and the passphrase as it's second parameter.
...
- #279 `e734010` Add -backupzcarbon startup flag. (presstab)
