...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CO2 and zcarbon money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CO2/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CO2/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CO2/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-pivstake=<n>", strprintf(_("Enable or disable staking functionality for CO2 inputs (0-1, default: %u)"), 1));
...
//    strUsage += "  -anonymizecarbonzeroamount=<n>     " + strprintf(_("Keep N CO2 anonymized (default: %u)"), 0) + "\n";
        ...
        do {
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZCO2Minted();
                        RecalculateZCO2Spent();
                    }
                    RecalculateCO2Supply(1);
                }
...
//    if (nAnonymizeCarbonZeroAmount < 2) nAnonymizeCarbonZeroAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1CO2+1000 == (.1CO2+100)*10
       10CO2+10000 == (1CO2+1000)*10
    */
