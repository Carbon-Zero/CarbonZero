...
CzcarbonTracker::CzcarbonTracker(std::string strWalletFile)
{
...
CzcarbonTracker::~CzcarbonTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzcarbonTracker::Init()
{
...
bool CzcarbonTracker::Archive(CMintMeta& meta)
{
...
bool CzcarbonTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzcarbonTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzcarbonTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzcarbonTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzcarbonTracker::GetSerialHashes()
{
...
CAmount CzcarbonTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzcarbonTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzcarbonTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzcarbonTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzcarbonTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzcarbonTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzcarbonTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzcarbonTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzcarbonTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzcarbonTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzcarbonTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzcarbonTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzcarbonTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzcarbonTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzcarbonTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzcarbonTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzcarbonTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzcarbon from DB\n", __func__, listDeterministicDB.size());
    }
