...
void PrivacyDialog::sendzcarbon()
{
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CO2 </b>will be added to the standard transaction fees!<br />";
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZCO2_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zcarbon require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zcarbon"));
            ui->TEMintStatus->repaint();
            return;
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzcarbon->setToolTip(tr("PrivacyDialog", "Enter an amount of CO2 to convert to zcarbon", 0));
