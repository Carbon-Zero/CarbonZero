...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total carbonzero balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zcarbonsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zcarbon denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zcarbon denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zcarbon denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zcarbon denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zcarbon denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zcarbon denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zcarbon denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zcarbon denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zcarbon denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in carbonzero/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in carbonzero/kb\n"
    ...
    UniValue zcarbonObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zcarbonObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zcarbonObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zcarbonsupply", zcarbonObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"carbonzeroaddress\"\n"
            "\nReturn information about the given carbonzero address.\n"

            "\nArguments:\n"
            "1. \"carbonzeroaddress\"     (string, required) The carbonzero address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"carbonzeroaddress\", (string) The carbonzero address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are carbonzero addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) carbonzero address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"carbonzeroaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"carbonzeroaddress\"  (string, required) The carbonzero address to use for the signature.\n"
