...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 2.5 * COIN;
    } else if (nHeight > 1600000 && nHeight <= 5000000) {
        nSubsidy = 2 * COIN;
    } else
        nSubsidy = 2 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCO2Stake)
{
    int64_t ret = 0;
...
bool RecalculateCO2Supply(int nHeightStart)
{
...
bool UpdateZCO2Supply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateCO2Supply(Params().Zerocoin_StartHeight());
    }

    //Track zcarbon money supply in the block index
    if (!UpdateZCO2Supply(block, pindex))
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CO2 to zcarbon
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZCO2() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
