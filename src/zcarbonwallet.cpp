...
bool CzcarbonWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBTCZCount(nCountLastUsed);
    else if (!walletdb.ReadZBTCZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzcarbonWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBTCZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzcarbonWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBTCZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzcarbonWallet::SeedToZBTCZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
void CzcarbonWallet::GenerateDeterministicZBTCZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
